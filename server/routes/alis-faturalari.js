    ...
    try {
        ...
        if (tier !== 'vip') {
            ...
            const countResult = await query(
                'SELECT COUNT(*) FROM alis_faturalari WHERE profile_id = $1 AND created_at >= $2',
            ...
            if (currentCount >= limit) {
                return res.status(403).json({
                    error: `Aylık alış faturası limitinize ulaştınız (${limit}). Daha fazla işlem için paketinizi yükseltin.`
                });
            }
        }
        // ---------------------
